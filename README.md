...
* [`src/snapcast_control/`](src/snapcast_control/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/snapcast_control/core.cljs): contains the SPA entry point, `init`
...
5. Clone this repo and open a terminal in the `snapcast-control` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/snapcast_control/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns snapcast-control.example
  (:require [snapcast-control.config :as config])
